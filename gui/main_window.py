...
# ==================== ДЕЛЕГАТЫ (ОБЪЯВЛЯЮТСЯ ДО MainWindow) ====================
...
class MainWindow(QMainWindow):
    ...
    def start_fleet(self):
        ...
        try:
            ...
            # 🔥 FIX: Запускаем потоки ожидания и мониторинга
    ...
    def wait_for_connection_and_plot(self):
        ...
        while self.is_running and (time.time() - start) < max_wait:
            if self.fleet and self.fleet.drones:
                # 🔥 FIX: Ждём пока хотя бы один дрон подключится (объект Pioneer создан)
    ...
    def create_all_plots(self):
        ...
        # 🔥 FIX: Принудительно обновляем контейнер
    ...
    def stop_fleet(self):
        # 🔥 FIX: Сбрасываем флаги принудительно
        self.is_running = False
        self.is_flying = False
